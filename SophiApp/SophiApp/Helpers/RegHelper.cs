...
namespace SophiApp.Helpers
{
    ...
    {        
        private static RegistryKey GetKey(RegistryHive hive, string keyPath) => RegistryKey.OpenBaseKey(hive, Environment.Is64BitOperatingSystem ? RegistryView.Registry64 : RegistryView.Registry32)
                                                                                            .OpenSubKey(keyPath);

        private static RegistryKey SetKey(RegistryHive hive, string keyPath) => RegistryKey.OpenBaseKey(hive, Environment.Is64BitOperatingSystem ? RegistryView.Registry64 : RegistryView.Registry32)
        ...
        internal static bool KeyExist(RegistryHive hive, string path, string name) => GetKey(hive, path).GetValueNames().Contains(name);
