...
namespace SophiApp.Customisations
{
    public static class CustomisationStatus
    {
        ...
        public static bool _326() => _327().Invert();

        public static bool _327() => RegHelper.GetStringValue(RegistryHive.CurrentUser, CONTROL_PANEL_USER_PROFILE_PATH, INPUT_METHOD_OVERRIDE) == INPUT_ENG_VALUE;

        public static bool _328() => OneDriveHelper.IsInstalled() ? throw new OneDriveIsInstalledException() : true;

        public static bool _329() => RegHelper.GetStringValue(RegistryHive.CurrentUser, USER_SHELL_FOLDERS_PATH, IMAGES_FOLDER)
                                        == RegHelper.GetStringValue(RegistryHive.CurrentUser, USER_SHELL_FOLDERS_PATH, _329_DESKTOP_FOLDER);

        public static bool _330() => _329().Invert();

        public static bool _332() => _333().Invert();

        public static bool _333() => RegHelper.GetNullableIntValue(RegistryHive.LocalMachine, WINDOWS_MITIGATION_PATH, MITIGATION_USER_PREFERENCE)
                                              .HasNullOrValue(_333_DEFAULT_VALUE);

        public static bool _334() => RegHelper.GetNullableIntValue(RegistryHive.CurrentUser, ADVANCED_EXPLORER_PATH, _334_SEPARATE_PROCESS)
                                              .HasNullOrValue(DISABLED_VALUE)
                                              .Invert();

        public static bool _335() => RegHelper.GetNullableIntValue(RegistryHive.LocalMachine, _335_RESERVE_MANAGER_PATH, _335_SHIPPED_RESERVES)
                                              .HasNullOrValue(ENABLED_VALUE);

        public static bool _336() => RegHelper.SubKeyExist(RegistryHive.CurrentUser, _336_TYPELIB_PATH)
                                              .Invert();

        public static bool _337() => RegHelper.GetStringValue(RegistryHive.Users, _337_DEFAULT_KEYBOARD_PATH, _337_INITIAL_INDICATORS) == _337_ENABLED_VALUE;

        public static bool _338() => RegHelper.KeyExist(RegistryHive.LocalMachine, _338_KEYBOARD_LAYOUT_PATH, _338_SCAN_CODE)
                                              .Invert();

        public static bool _339() => RegHelper.GetStringValue(RegistryHive.CurrentUser, _339_STICKY_KEYS_PATH, _339_FLAGS)
                                              .HasNullOrValue(_339_ENABLED_VALUE);

        public static bool _340() => RegHelper.GetNullableIntValue(RegistryHive.CurrentUser, _340_AUTOPLAY_HANDLERS_PATH, _340_AUTOPLAY)
                                              .HasNullOrValue(_340_ENABLED_VALUE);

        public static bool _341() => RegHelper.GetNullableIntValue(RegistryHive.LocalMachine, _341_THUMBNAIL_CACHE_PATH, _341_AUTOPLAY)
                                              .HasNullOrValue(_341_ENABLED_VALUE);

        public static bool _342() => RegHelper.GetNullableIntValue(RegistryHive.CurrentUser, WINLOGON_PATH, _342_RESTART_APPS)
                                              .HasNullOrValue(DISABLED_VALUE)
                                              .Invert();

        public static bool _343() => DomainHelper.PcInDomain().Invert()
                                     ? FirewallHelper.IsRuleGroupEnabled(_343_FILE_PRINTER_SHARING_GROUP) && FirewallHelper.IsRuleGroupEnabled(_343_NETWORK_DISCOVERY_GROUP)
                                     : throw new PcJoinedToDomainException();

        public static bool _344() => RegHelper.GetNullableIntValue(RegistryHive.LocalMachine, UPDATE_UX_SETTINGS_PATH, _344_ACTIVE_HOURS)
                                              .HasNullOrValue(_344_AUTO_STATE);

        public static bool _345() => RegHelper.GetNullableIntValue(RegistryHive.LocalMachine, UPDATE_UX_SETTINGS_PATH, _345_IS_EXPEDITED)
                                              .HasNullOrValue(DISABLED_VALUE)
                                              .Invert();
        ...
        public static bool _350()
        {
            var vcVersions = WebHelper.GetJsonResponse(_350_VC_VERSION_URL, new VCRedistrDto());
            var latestVersion = vcVersions.Supported.Where(item => item.Name == _350_VC_REDISTR_FOR_VS_2022 && item.Architecture == X64).Select(item => item.Version).First();
        ...
        public static bool _352() => OneDriveHelper.IsInstalled() ? throw new OneDriveIsInstalledException() : false;

        public static bool _353() => OneDriveHelper.IsInstalled() ? false : throw new OneDriveNotInstalledException();
